...
folderWithZipFilesToUpload <- "D:\\studyResults\\dmePaper\\"
...
data <- incidenceRate |>
  ...
  dplyr::mutate(
    ...
    calendarYear = as.Date(paste0(calendarYear, "-06-01"))
  ) |>
  dplyr::select(
    cohortId,
    databaseId,
  ...
  dplyr::inner_join(
    data |>
      dplyr::group_by(cohortId,
                      databaseId) |>
      dplyr::summarise(baselineIncidenceRate = sum(cohortCount) / sum(personYears)) |>
      dplyr::ungroup()
  )
